  ...
  return (
    <div className="min-h-screen bg-[#fafaf8] text-[#1a1a1a] font-sans overflow-x-hidden">
      ...
      <section className="max-w-5xl mx-auto px-12 pt-24 pb-20 relative">
        <div className="absolute right-12 top-20 w-80 h-80 rounded-full bg-green-700/5 pointer-events-none" />
        <span className="inline-block text-[11px] font-medium tracking-widest uppercase text-green-700 border border-green-200 rounded-full px-4 py-1 mb-7">
          About Autoflow
        </span>
        <h1 className="font-extrabold text-[clamp(42px,6vw,80px)] leading-[1.05] tracking-tight text-[#111] max-w-3xl">
          ...
          <em className="not-italic text-green-700 font-semibold">already</em> love.
        </h1>
        <p className="mt-7 text-lg font-light text-[#555] leading-relaxed max-w-lg">
          Autoflow was built on one idea — your apps should talk to each other, so you don't have to.
      ...
      <div className="border-t border-b border-[#e8e6e0] max-w-5xl mx-auto px-12 py-12 grid grid-cols-4">
        {stats.map((s, i) => (
          <div
            key={i}
            className={`px-8 ${i !== 0 ? "border-l border-[#e8e6e0]" : "pl-0"}`}
          >
            <div className="text-[44px] font-extrabold tracking-tight leading-none text-[#111]">
              {s.num.replace(/[+%k]$/, "")}
              <span className="text-green-700">{s.num.match(/[+%k]$/)?.[0]}</span>
            </div>
      ...
      <section className="max-w-5xl mx-auto px-12 py-24 grid grid-cols-2 gap-20 items-center">
        ...
        <div className="text-base font-light text-[#555] leading-relaxed space-y-4">
          <p>
            We started Autoflow because we were tired of copying data between apps manually.
            There had to be a better way — and there was.
          </p>
          <p>
            Today, Autoflow lets anyone build powerful automations in minutes. Whether you're a
      ...
      <div className="max-w-5xl mx-auto px-12 pb-20">
        <div className="bg-[#111] rounded-2xl p-10 relative overflow-hidden">
          <div className="absolute -top-10 -right-10 w-48 h-48 rounded-full bg-green-700/10 pointer-events-none" />
          ...
          <div className="flex items-center gap-3 flex-wrap">
            {["📧 Gmail", "📋 Notion", "💬 Slack"].map((app, i) => (
              <>
                {i > 0 && (
                  <span key={`arrow-${i}`} className="text-green-500 text-lg">→</span>
                )}
                {i === 1 ? (
                  <div key={app} className="bg-green-700 text-white rounded-xl px-4 py-2.5 text-[13px] font-semibold">
                    ⚡ Autoflow
                  </div>
                ) : null}
        ...
        <div className="max-w-5xl mx-auto">
          <div className="flex justify-between items-end mb-14">
            ...
            <p className="text-[14px] text-[#666] max-w-[260px] text-right leading-relaxed">
              Three principles that guide every decision we make at Autoflow.
            </p>
          </div>
          <div className="grid grid-cols-3 gap-0.5">
            {values.map((v) => (
              ...
              >
                <div className="text-[11px] font-semibold text-green-500 tracking-widest mb-5">
                  {v.num}
                </div>
      ...
      <section className="max-w-5xl mx-auto px-12 pb-20">
        <div className="bg-green-700 rounded-3xl px-16 py-16 flex justify-between items-center gap-10">
          <h2 className="text-[36px] font-extrabold text-white tracking-tight leading-[1.15] max-w-md">
            Ready to automate your workflow?
          </h2>
          <button
            onClick={() => router.push("/signup")}
            className="flex-shrink-0 bg-white text-green-700 font-bold text-[15px] rounded-full px-9 py-4 hover:-translate-y-0.5 hover:shadow-lg transition-all"
          >
            Get started free
          </button>
        </div>
